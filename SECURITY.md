...
4. 查看 `clean --json` 中的 Observation、CandidateAction、风险、影响、阻断理由和快照指纹。
5. 使用 `--thread-id` 从单条唯一目标开始，或把完整 `--action-id` 用作自动化选择器。
6. 核对保存位置、完整对话 ID、列表记录、内容文件，以及将一同删除的关联任务对话 ID。
7. 确认没有活跃前端引用，并备份完整保存位置。
8. 在 TTY 中输入明确确认词 `确认删除`，或对已显式选择的目标使用 `--yes`；程序仍会在删除前重新扫描并比较动作快照与精确范围。
...
自动化不得保存临时编号，应使用计划 JSON 中的完整 action ID。`--yes` 只跳过最终确认提示，不会选择目标、放宽风险、忽略扫描失败或绕过快照与范围重验证。
...
- 扫描前端 SQLite 时使用只读连接；
- 不直接修改 Codex `state_5.sqlite`、旧索引或日志表；
- 不直接删除 rollout JSONL；
