...
项目目前处于 **Alpha**。`scan` 始终只读。`clean` 会先生成基于动作的结构化计划；只有用户明确选择目标、计划通过删除前重验证，并在 TTY 中输入确认词或使用 `--yes` 跳过该确认提示，才会执行不可恢复的硬删除。
...
所有发现都会成为 Observation，并显示一个或多个 CandidateAction。当前只有 `delete_conversation`（删除整条对话）执行器可用；修复列表路径、隔离文件、清除关系或前端引用等动作仅结构化显示，并附带不可用原因。
...
交互查看所有问题。输出按保存位置和风险分类，每个候选动作都有临时编号：
...
TTY 交互会在同一进程中展示最终计划，然后要求输入明确确认词 `确认删除`；确认后立即重新扫描并执行。输入其他内容或遇到 EOF 都会取消且零修改。`--yes` 只跳过这个最终确认提示，不会跳过编号/目标选择。
...
- 启动 app-server 后、任何删除请求前会再次捕获完整范围；与已批准范围不完全一致时整组停止且发送零个删除请求；
...
`clean --json` 在计划阶段包含保存位置、Observation、候选动作、风险、影响、阻断原因、action ID 和快照指纹，例如：
...
人类可读输出会缩写对话 ID；JSON 始终保留完整 ID、action ID 和绝对路径。同一 ID 位于不同 Codex 数据目录时是不同目标，不会静默合并。绝对路径、工作目录、会话 ID 与错误信息可能泄露用户名、项目名或目录结构，分享前请脱敏。

## 退出码

| 退出码 | 含义 |
|---:|---|
| `0` | 扫描成功，或所有已尝试删除均经验证成功 |
| `1` | 扫描、选择、重验证、Codex app-server、删除或删除后验证发生错误 |
| `2` | 非 TTY 的 `clean` 未传 `--yes`，仅预览计划，没有删除 |
...
- 路径错位、重复文件和残留关联记录不应通过手工“删一个文件”解决；单独关系修复等动作目前只显示、不执行。残留记录指向的子对话只有在身份、来源、文件状态和精确范围均通过窄授权时，才可逐项执行整条删除。
